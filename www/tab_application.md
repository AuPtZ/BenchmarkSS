...
1. **Single method** : Query drugs by one of SSMs, as the **traditional way**. Typically, **abs(logFC) > ±1** is used for filter differential expression genes.  
...
we can select some methods and direction to rank the drugs, generally, if we upload a oncogenic signature, chose **down** , else, chose  **up**   
